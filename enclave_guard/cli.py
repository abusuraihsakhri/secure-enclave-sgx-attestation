"""
Command-Line Interface for EnclaveGuard: Intel SGX / AMD SEV Remote Attestation & Quoting Agent.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="secure-enclave-sgx-attestation", description="EnclaveGuard: Intel SGX / AMD SEV Remote Attestation & Quoting Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  ENCLAVEGUARD: INTEL SGX / AMD SEV REMOTE ATTESTATION & QUOTING AGENT")
        print(f"  Domain: Privacy-Preserving Federated Healthcare & FHE | Standard: Intel SGX ECDSA Attestation / AMD SEV-SNP")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting EnclaveGuard: Intel SGX / AMD SEV Remote Attestation & Quoting Agent on http://{args.host}:{args.port}")
